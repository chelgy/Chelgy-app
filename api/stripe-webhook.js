...
export default async function handler(req, res) {
  ...
  // A domain was bought through Chelgy via api/domain-checkout.js. Vercel's registrar
  // is ASYNCHRONOUS: POST /buy returns an orderId once the order is PLACED, not once
  // the domain is registered (the order goes draft -> purchasing -> completed | failed).
  // So we do NOT connect the domain here anymore — that was the bug that pointed sites
  // at domains that never actually registered. Instead we record the purchase as
  // "pending" and let api/domain-reconcile.js confirm it completed (then connect it) or
  // failed (then refund automatically).
  ...
  async function registerDomain(s, meta) {
    ...
    // Idempotency: Stripe retries checkout.session.completed. If we've already recorded
    // this exact checkout session, do NOT place a second order (that would double-buy).
    try {
      const chk = await fetch(SUPABASE_URL + "/rest/v1/domains?select=id&session_id=eq." + encodeURIComponent(s.id || "x") + "&limit=1", { headers: { apikey: SERVICE, Authorization: "Bearer " + SERVICE } });
      const ex = await chk.json().catch(() => []);
      if (Array.isArray(ex) && ex.length) return;
    } catch { /* if the check fails, fall through — the reconciler still keys off the order */ }
    ...
    try {
      ...
      // Order couldn't even be placed -> nothing was bought -> refund and stop.
      if (!br.ok || !bj.orderId) { await refund(); return; }

      // Order PLACED (not yet complete). Record it as pending with everything the
      // reconciler needs to finish or refund it. We deliberately do NOT set the site's
      // custom_domain here — the reconciler does that only once Vercel confirms the
      // registration actually completed.
    ...
    } catch { /* swallow: the order may have gone through; the reconciler will catch up from the pending row */ }
  }

  // A domain the member owns was renewed via api/domain-renew.js. Same async story as a
  // purchase: /renew returns an orderId when the renewal is PLACED, not when it's done.
  // So we mark the domain "renewing" and let api/domain-reconcile.js push the expiry out
  // a year only once Vercel confirms it completed — or refund if it failed. (Previously
  // this bumped the expiry the moment an orderId came back, so a failed renewal silently
  // looked successful.)
  async function renewDomain(s, meta) {
    ...
    // Idempotency: don't place a second renewal if Stripe retries this session.
    try {
      const chk = await fetch(SUPABASE_URL + "/rest/v1/domains?select=id&session_id=eq." + encodeURIComponent(s.id || "x") + "&limit=1", { headers: { apikey: SERVICE, Authorization: "Bearer " + SERVICE } });
      const ex = await chk.json().catch(() => []);
      if (Array.isArray(ex) && ex.length) return;
    ...
    try {
      ...
      // Renewal PLACED (not complete). Flag the row; the reconciler extends the expiry
      // once Vercel confirms completion, or refunds on failure. Expiry is NOT bumped here.
      await fetch(SUPABASE_URL + "/rest/v1/domains?domain=eq." + encodeURIComponent(domain), {
        method: "PATCH",
        headers: { "apikey": SERVICE, "Authorization": "Bearer " + SERVICE, "Content-Type": "application/json", "Prefer": "return=minimal" },
        body: JSON.stringify({ status: "renewing", order_id: rj.orderId, payment_intent: s.payment_intent ? String(s.payment_intent) : null, session_id: s.id || null, pending_since: new Date().toISOString() })
      });
    } catch { /* swallow */ }
  }
